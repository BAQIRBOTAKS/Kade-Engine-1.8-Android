...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (isSM)
			executeModchart = FileSystem.exists(Main.dataPath() + pathToSm + "/modchart.lua");
		if (executeModchart)
	...
	public function generateSong(dataPath:String):Void
	{
		...
		if (!paused)
		{
			...
			if (!isStoryMode && isSM)
			{
				trace("Loading " + Main.dataPath() + pathToSm + "/" + sm.header.MUSIC);
				var bytes = File.getBytes(Main.dataPath() + pathToSm + "/" + sm.header.MUSIC);
				var sound = new Sound();
